...
```canon-result
{ "outcome": "<one of: approved, blocked>", "summary": "<one sentence>", "route": [] }
```
...
## Your stage: Release Gatekeeper

Task: Verify the release packet is complete and every gate has passed, then record an approve or block decision.

Done when: The decision names the evidence checked, and a blocked release states exactly what is missing.
...
### Handoff contract
- release-notes-author handed over `.canon/artifacts/run-muagj3le/release-notes-author/release_notes.md` (approved) at commit ffef9fdb3361.
You are working at commit ffef9fdb3361.

### Input from Release Notes Author (release_notes)
Outcome: approved
Summary: Release notes, changelog, rollback plan, and release checklist prepared for rocket-asteroid collision animation release.
