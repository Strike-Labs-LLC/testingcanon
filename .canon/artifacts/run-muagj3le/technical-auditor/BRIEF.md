...
## Your stage: Technical Auditor

Task: Audit the full packet for quality and production risk, and record an approval or specific findings.

Done when: Migrations, rollback, monitoring, and blast radius are each assessed in writing, and the outcome is one clear decision.
...
### Handoff contract
- security-reviewer handed over `.canon/artifacts/run-muagj3le/security-reviewer/security_report.md` (passed) at commit ffef9fdb3361.
You are working at commit ffef9fdb3361.

### Input from Security Reviewer (security_report)
Outcome: passed
Summary: Security audit passed with zero vulnerabilities, secrets, or insecure dependencies found in the rocket explosion animation diff.
