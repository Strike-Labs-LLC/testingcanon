...
//
//   node .canon/release.mjs candidate --issue <run issue>
//   node .canon/release.mjs authorize --issue <run issue> [--sha <sha>]
//   node .canon/release.mjs verify --sha <sha>
//   node .canon/release.mjs publish --issue <run issue> --tag <tag> --sha <sha>
//
 ...
 * `.canon/merge.mjs` may land. It never consults `releaseSha`: asking "what did the
 ...
 * released from the commit `.canon/merge.mjs` recorded after GitHub accepted a
...
if ((process.argv[1] ?? "").endsWith("release.mjs")) {
  ...
  if (!handler) {
    console.error(
      "Usage: node .canon/release.mjs <candidate|authorize|verify|publish> [--issue N] [--sha SHA] [--tag TAG]",
    );
    process.exit(1);
  }
  handler().catch((error) => {
