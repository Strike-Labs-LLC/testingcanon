...
/**
 * True when the handoff came back up the pipeline: the stage that raised it sits
 * after the stage receiving it. Pipeline order is the order the graph declares.
 */
export function isLoopBack(graph, fromId, toId) {
  const order = (graph.stages ?? []).map((stage) => stage.id);
...
/**
 * The standing facts about this repository, as the package recorded them.
 *
 * Agent contracts embed the same text, but a stage whose contract is missing or
 * stale would otherwise run blind, so the brief reads the file at run time.
 */
const CONTEXT_HEADINGS = {
  repositoryResponsibility: "What this repository owns",
  productOutcome: "What the product is for",
  primaryUsers: "Who uses it",
  decisionPriorities: "How to resolve tradeoffs",
  mustPreserve: "Must keep working",
  mustNotDo: "Never do this",
  nonGoals: "Out of scope",
};

export function repositoryContextSection(root = CANON_ROOT) {
  ...
  const entries = Object.entries(CONTEXT_HEADINGS)
    .map(([key, heading]) => [heading, String(context?.[key] ?? "").trim()])
    .filter(([, value]) => value);
  ...
  return [
    ...
    "",
    ...entries.map(([heading, value]) => `### ${heading}\n\n${value}\n`),
  ].join("\n");
}

/** The ticket that started the run, quoted as data rather than as instructions. */
function sourceTicketSection(run) {
  const source = run.source;
  const body = String(source?.body ?? "").trim();
  if (!source || !body) return "";
  return [
    `## The ticket (#${source.number})`,
    "",
...
function upstreamContext(graph, run, stage) {
  ...
  if (handoffs.length) {
    ...
    //
    // Only loop-backs. A forward handoff's artifact is already rendered below as
    // "Input from …", and quoting it twice both wastes the model's context and
    // reads to the agent as a second, separate complaint about the same work.
    const findings = handoffs
      .filter((h) => isLoopBack(graph, h.from, stage.id))
...
async function main() {
  ...
  if (!valid) {
    ...
    await publish(
      ...
      stage.id,
      resultComment(
        { ...provenance(run, stage), evaluatedSha: evaluatedSha || null, status: "failed", summary: reason },
