...
/**
 * Stop a run and record why, on the record itself.
 *
 * `status: "failed"` on its own told a reader nothing. A run that died in
 * dispatch looked exactly like one whose last stage was rejected, and the
 * reason lived only in an issue comment or a workflow log — neither of which
 * the record carries. Every reader of a run already has the record, so the
 * reason belongs there.
 *
 * `reason` is one sentence a person can act on. `detail` carries the specific
 * failures behind it (the preflight list, for instance), `code` says which
 * kind of failure it was, and `stage` names the stage when one is to blame.
 * The timeline entry stays with the caller: the events are already named more
 * precisely there than this could name them.
 *
 * The first failure recorded is kept. Anything after it is a consequence of a
 * run that had already stopped, and the cause is what a reader needs.
 */
export function failRun(run, reason, extra = {}) {
  run.status = "failed";
  if (run.failure) return run;
  run.failure = {
    code: extra.code ?? null,
    reason: String(reason ?? "").trim() || "The run stopped without recording a reason.",
    detail: (extra.detail ?? []).map((line) => String(line)).filter(Boolean),
...
export function createRun(graph, meta = {}) {
  ...
  const run = {
    ...
    // Why the run stopped, when it stopped badly. Null while it is healthy.
    failure: null,
    stages,
    edgeCounts: {},
...
export function recordOutcome(graph, run, stageId, result) {
  ...
  if (result.status === "failed") {
    ...
    failRun(run, result.summary || `${stage.name} reported an execution failure.`, {
      code: "stage-failed",
      stage: stageId,
    });
  ...
  if (candidate && evaluated && evaluated !== candidate) {
    ...
    state.finishedAt = now();
    failRun(run, `${stage.name} judged a commit that is no longer the candidate.`, {
  ...
  for (const edge of outbound(graph, stageId)) {
    ...
    if (edge.kind === "return") {
      ...
      if (count > limit) {
        failRun(run, `${stage.name} sent the work back more than ${limit} times.`, {
          code: "loop-limit",
          stage: stageId,
          detail: [`Edge ${edge.id} was taken ${count} times; the limit is ${limit}.`],
        });
...
export function failUnreportedStages(run) {
  ...
  for (const [id, state] of Object.entries(run.stages ?? {})) {
    ...
    state.finishedAt = now();
    failRun(run, "A stage job ended without publishing a trusted result.", {
      code: "stage-no-result",
      stage: id,
      detail: [`${id} was dispatched but never reported back.`],
    });
