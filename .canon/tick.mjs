...
//
//   node .canon/tick.mjs plan    --issue N   pick the ready stages, assign human gates
//   node .canon/tick.mjs advance --issue N   apply posted results, route, re-dispatch
//
...
async function requireIntactTrail(issueNumber, graph, run) {
  ...
  failRun(run, "The run record was edited outside Canon, so the run was halted.", {
    code: "audit-trail",
    detail: [`Transition ${check.at ?? "?"} no longer matches its recorded hash: ${check.reason}.`],
  });
