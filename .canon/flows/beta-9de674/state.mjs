 ...
 *   5. it evaluated the run's current candidate commit.
 *
...
export function trustedResults(comments, run, cursor = run?.cursor ?? 0) {
  ...
  for (const item of comments) {
    ...
    if (candidate && !payload.evaluatedSha) {
      rejected.push({ ...entry, author, reason: "it names no evaluated commit" });
      continue;
    }
    if (candidate && payload.evaluatedSha !== candidate) {
      rejected.push({
        ...entry,
        author,
 ...
 * `.canon/flows/beta-9de674/release.mjs` reads this stamp; nothing else may authorize a release.
 *
...
export function stampRelease(run, graph = null) {
  ...
  run.release = {
    authorized: true,
    sha,
    // Set by `.canon/flows/beta-9de674/merge.mjs` once the candidate actually lands on the default
