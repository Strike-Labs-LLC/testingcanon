...
Generated runtime for **Bug triage fix verify**.

Edit the blueprint in Canon and recompile. Do not edit these files by hand.

This flow is installed in its own namespace, `.canon/flows/beta-9de674`, with workflows named `canon-beta-9de674-*.yml`. Other flows in this repository have their own namespaces and never read or write this one.
...
1. Actions → **Orchestration pipeline — Bug triage fix verify** → Run workflow, or
2. Add the `canon:start:beta-9de674` label to an issue.

The run issue is created immediately, labelled `canon-run:beta-9de674`. Every step is recorded there.
