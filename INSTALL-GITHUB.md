...
This package configures a GitHub repository to run the **Autonomous production engineering** pipeline you designed.
...
## Install the Canon GitHub App — required

Every run asks Canon to authorize it and hands back a short-lived, operation-scoped
token. Canon can only answer if its GitHub App is installed on `YOUR-ORGANIZATION/YOUR-REPOSITORY`. Without
the App, a run stops at its first step with an authorization failure — the
repository's built-in `GITHUB_TOKEN` cannot stand in for it.

1. Install the Canon GitHub App on `YOUR-ORGANIZATION/YOUR-REPOSITORY`.
...
Work through all four. A run started before these are in place fails, and the
failure is a setting, not your pipeline.

- [ ] The Canon GitHub App is installed on `YOUR-ORGANIZATION/YOUR-REPOSITORY` and Canon reports it as connected.
- [ ] GitHub Copilot is enabled for the organization and for this repository, the
      **Copilot CLI** policy is on, and the seat covers the account the workflows run
      as. Without it, every agent stage fails with *Access denied by policy settings*.
      Set the model each stage uses under Copilot settings.
- [ ] Run `bash .sdlc/github/labels.sh` so every label the workflows apply exists.
- [ ] Open Settings → Rules and confirm the live branch ruleset requires only the
      checks this package emits. A required check no workflow produces blocks every
      merge forever.
